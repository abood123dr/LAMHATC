...
import { StockMovement } from "@/api/base44Client";
...
export default function Movements() {
  ...
  const { data: movements = [] } = useQuery({
    queryKey: ["movements"],
    queryFn: () => StockMovement.list('-created_at', 500),
  });
  ...
  return (
    <div>
      <PageHeader title="سجل حركة المخزون" subtitle="جميع حركات الدخول والخروج لكل فرع" />
      <div className="mb-5">
      ...
      {filtered.length === 0 ? (
        <EmptyState icon={History} title="لا توجد حركات بعد" description="ستظهر هنا كل حركات المخزون تلقائيًا عند تسجيل المبيعات" />
      ) : (
              ...
              return (
                <div key={m.id} className="flex items-center gap-4 p-4 hover:bg-accent/30">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5 truncate">{cfg.label}{m.notes && ` · ${m.notes}`}</p>
                  </div>
                  <div className="text-left shrink-0">
                    <p className={cn("font-bold text-sm", m.movement_type === "out" ? "text-rose-600" : "text-emerald-600")}>{m.movement_type === "out" ? "−" : "+"}{m.quantity}</p>
                    <p className="text-[11px] text-muted-foreground whitespace-nowrap">رصيد: {m.balance_after}</p>
                  </div>
                  <div className="hidden md:block text-xs text-muted-foreground whitespace-nowrap shrink-0">
                    {m.created_at && format(new Date(m.created_at), "d MMM · HH:mm", { locale: ar })}
                  </div>
                </div>
              );
            })}
