...
export default function Sales() {
  ...
  const { data: sales = [] } = useQuery({
    queryKey: ["sales"],
    queryFn: () => db.entities.Sale.list("-sale_date", 500),
  });
  ...
  const createMut = useMutation({
    mutationFn: async (data) => {
      // Create sale
      const sale = await db.entities.Sale.create(data);
      ...
      if (product) {
        ...
        await db.entities.Product.update(product.id, { [key]: newStock });
        // Log movement
    ...
    onSuccess: (sale) => {
      ...
      setFormOpen(false);
      const motivations = [
        "🔥 رائع! بيعة جديدة تُضاف لرصيدك!",
        "🎯 أحسنت! استمر في هذا الإيقاع المذهل!",
        "⭐ بيعة ناجحة! أنت على الطريق الصحيح!",
        "🚀 ممتاز! كل بيعة تقربك من هدفك!",
        "💪 عظيم! مبيعاتك تتصاعد!",
      ];
      const msg = motivations[Math.floor(Math.random() * motivations.length)];
      toast.success(msg, {
        description: `${sale.product_name} — ${sale.total_amount ? sale.total_amount.toLocaleString("ar-SA") : ""} ${sale.branch === "saudi" ? "ر.س" : "₺"}`,
        duration: 4000,
      });
    },
  });
  ...
  const deleteMut = useMutation({
    mutationFn: async (sale) => {
      await db.entities.Sale.delete(sale.id);
      // Restore stock
  ...
  return (
    <div>
      <PageHeader
        ...
        action={
          <Button
            ...
            className="hidden md:flex bg-gold hover:bg-gold-dark text-white gap-2"
          >
      ...
      <button
        onClick={() => setFormOpen(true)}
        className="md:hidden fixed bottom-20 left-4 z-40 w-14 h-14 bg-gold hover:bg-gold-dark text-white rounded-full shadow-xl flex items-center justify-center active:scale-95 transition-all"
      >
      ...
      <div className="bg-card border border-border rounded-2xl shadow-luxe overflow-hidden">
        ...
        {filtered.length === 0 ? (
          <EmptyState
            ...
            action={
              <Button onClick={() => setFormOpen(true)} className="bg-gold hover:bg-gold-dark text-white gap-2">
                <Plus className="w-4 h-4" /> تسجيل بيع
              </Button>
            }
          />
        ) : (
                ...
                <div key={s.id} className="px-4 py-4 flex items-center gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {s.installation_images?.length > 0 && (
                        <span className="inline-flex items-center gap-1 text-[10px] text-gold bg-accent px-1.5 py-0.5 rounded-full shrink-0">
                          <Images className="w-3 h-3" />{s.installation_images.length}
                        </span>
                      )}
                    </div>
                    ...
                    <tr key={s.id} className="hover:bg-accent/30 transition-colors text-sm">
                      <td className="px-5 py-3 font-medium">
                        <div className="flex items-center gap-2">
                          {s.product_name}
                          {s.installation_images?.length > 0 && (
                            <span className="inline-flex items-center gap-1 text-[10px] text-gold bg-accent px-1.5 py-0.5 rounded-full">
                              <Images className="w-3 h-3" />{s.installation_images.length}
                            </span>
                          )}
                        </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              سيتم حذف العملية وإعادة الكمية إلى المخزون.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
