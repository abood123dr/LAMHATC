...
export default function DiscountCodes() {
  ...
  const { data: codes = [] } = useQuery({
    ...
    queryFn: () => db.entities.DiscountCode.list("-created_date", 200),
  });

  const saveMut = useMutation({
    mutationFn: (data) => editing
      ? db.entities.DiscountCode.update(editing.id, data)
      : db.entities.DiscountCode.create(data),
    onSuccess: () => {
  ...
  const deleteMut = useMutation({
    mutationFn: (id) => db.entities.DiscountCode.delete(id),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["discount-codes"] }); toast.success("تم حذف الكود"); },
  });

  const toggleMut = useMutation({
    mutationFn: ({ id, is_active }) => db.entities.DiscountCode.update(id, { is_active }),
  ...
  return (
    <div>
      <PageHeader
        ...
        action={
          <Button onClick={() => { setShowForm(true); setEditing(null); setForm(EMPTY_FORM); }} className="gap-2">
            <Plus className="w-4 h-4" /> إنشاء كود جديد
          </Button>
        }
      />
      ...
      {showForm && (
        <div className="bg-card border border-border rounded-2xl p-5 mb-6 shadow-luxe">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-semibold text-muted-foreground mb-1 block">الكود *</label>
              <input value={form.code} onChange={e => setForm({ ...form, code: e.target.value.toUpperCase() })}
                placeholder="مثال: SAVE20" dir="ltr"
            ...
            <div>
              <label className="text-xs font-semibold text-muted-foreground mb-1 block">نوع الخصم *</label>
            ...
            <div>
              <label className="text-xs font-semibold text-muted-foreground mb-1 block">قيمة الخصم *</label>
              <input type="number" value={form.discount_value} onChange={e => setForm({ ...form, discount_value: e.target.value })}
                placeholder={form.discount_type === "percentage" ? "مثال: 20 (يعني 20%)" : "المبلغ"}
                className="w-full px-4 py-2.5 rounded-xl border border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
            </div>
            <div>
              <label className="text-xs font-semibold text-muted-foreground mb-1 block">الفرع</label>
            ...
            <div>
              <label className="text-xs font-semibold text-muted-foreground mb-1 block">الحد الأقصى للاستخدام</label>
              <input type="number" value={form.max_uses} onChange={e => setForm({ ...form, max_uses: e.target.value })}
                placeholder="اتركه فارغاً = غير محدود"
                className="w-full px-4 py-2.5 rounded-xl border border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
            </div>
            <div>
              <label className="text-xs font-semibold text-muted-foreground mb-1 block">تاريخ الانتهاء</label>
          ...
          <div className="flex gap-2 mt-4">
            <Button onClick={handleSave} disabled={saveMut.isPending} className="gap-2">
              {saveMut.isPending ? "جاري الحفظ..." : editing ? "تحديث" : "إنشاء"}
            </Button>
                  ...
                  return (
                    <tr key={code.id} className="hover:bg-accent/30 transition-colors text-sm">
                      <td className="px-5 py-3">
                        <span className="font-black text-foreground font-mono tracking-wider">{code.code}</span>
                      </td>
                      ...
                      <td className="px-3 py-3 text-muted-foreground text-xs">
                        {code.expiry_date ? format(new Date(code.expiry_date), "d MMM yyyy", { locale: ar }) : "—"}
                      </td>
                      <td className="px-3 py-3">
                        <button onClick={() => toggleMut.mutate({ id: code.id, is_active: !code.is_active })}
                          className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-xs font-semibold transition-all hover:opacity-80 ${
                            effective ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"
                          }`}>
                          {effective ? <CheckCircle2 className="w-3.5 h-3.5" /> : <XCircle className="w-3.5 h-3.5" />}
                          {effective ? "مفعّل" : expired ? "منتهي" : maxed ? "نفد" : "معطّل"}
                        </button>
                      </td>
                      <td className="px-3 py-3">
                        <div className="flex items-center gap-1">
                          <button onClick={() => startEdit(code)} className="p-1.5 rounded-lg hover:bg-accent transition-colors">
                            <Edit2 className="w-3.5 h-3.5 text-muted-foreground" />
                          </button>
                          <button onClick={() => deleteMut.mutate(code.id)} className="p-1.5 rounded-lg hover:bg-red-50 transition-colors">
